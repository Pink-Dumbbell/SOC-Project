...
CORRELATE_STATE_FILE = "/app/data/correlate_state.json"
...
def _load_history():
    """컨테이너 재시작 시 저장해둔 상관분석 이력을 불러온다."""
    global ip_event_history
    if os.path.exists(CORRELATE_STATE_FILE):
        try:
            with open(CORRELATE_STATE_FILE, "r", encoding="utf-8") as f:
                raw = json.load(f)
            for src_ip, events in raw.items():
                ip_event_history[src_ip] = [
                    {"timestamp": datetime.fromisoformat(e["timestamp"]), "label": e["label"]}
                    for e in events
                ]
            print(f"[상관분석 이력 로드] {len(ip_event_history)}개 IP")
        except Exception as e:
            print(f"[상관분석 이력 로드 실패] {e}")


def _save_history():
    """현재 상관분석 이력을 파일로 저장한다 (재시작해도 유지되도록)."""
    os.makedirs(os.path.dirname(CORRELATE_STATE_FILE), exist_ok=True)
    with lock:
        raw = {
            src_ip: [{"timestamp": e["timestamp"].isoformat(), "label": e["label"]} for e in events]
            for src_ip, events in ip_event_history.items()
        }
    try:
        with open(CORRELATE_STATE_FILE, "w", encoding="utf-8") as f:
            json.dump(raw, f, ensure_ascii=False, indent=2)
    except Exception as e:
        print(f"[상관분석 이력 저장 실패] {e}")
